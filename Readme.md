...
- [CoinNudge](https://coinnudge.site/data) | `REST API`, `Parquet` | - Crypto market-research datasets with a free no-key JSON/CSV tier covering market breadth, funding rates, open interest, liquidations and listings, plus a versioned daily event-study release addressed by immutable release ID and delivered by self-service API key, JSON/CSV/Parquet files or a browser workspace. Archived rows carry event, observation and ingestion timestamps. [Samples](https://coinnudge.site/data/samples) | [API docs](https://coinnudge.site/data/api-docs)
